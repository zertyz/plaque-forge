...
use crate::{
    ...
    cli::{CreateSceneArgs, ExportTrajectoryArgs, PlaceSurfaceArgs, PlacementSpace},
    scene::{
        SCENE_FORMAT, Scene, SurfaceProposal, relative_reference, scene_document,
        trajectory_document, write_scene,
    },
    surface::Surface,
    video, workspace,
};

pub fn create(args: CreateSceneArgs) -> Result<()> {
    ...
    let alternatives = report
        ...
        .unwrap_or_default();
    let contents = scene_document(&args.input, &output, "ensemble", proposal, &alternatives)?;
    write_scene(&output, &contents, args.force)?;
    let Some(report) = report else {
        bail!(
            "automatic plaque detection found no plausible candidate; scene written to {}",
            output.display()
        );
    };
    println!(
    ...
    println!("scene: {}", output.display());
    Ok(())
}

fn to_proposal(candidate: &candidate::Candidate) -> SurfaceProposal {
    SurfaceProposal {
...
pub fn place_surface(args: PlaceSurfaceArgs) -> Result<()> {
    if !args.input.is_file() {
        bail!(
            "input video does not exist or is not a file: {}",
            args.input.display()
    ...
    if output.exists() && !args.force {
        bail!(
            "refusing to overwrite scene {}; use --force only when replacing the whole plaque-placement scene is intentional",
            output.display()
        );
    }
    ...
    let (space, depth) = match args.space {
        PlacementSpace::ScenePlane => ("scene-plane", "automatic"),
        PlacementSpace::ScreenCanvas => ("screen-canvas", "flat"),
    };
    let text = format!(
        "# Plaque Forge scene intent. Generated evidence belongs in the analysis cache.\n\
         format = {SCENE_FORMAT:?}\n\
         source = {}\n\
    ...
    let parsed: Scene =
        toml::from_str(&text).context("generated injected-plaque scene is not valid TOML")?;
    parsed
        .validate()
        .context("generated injected-plaque scene is invalid")?;
    let output_name = output
        .file_name()
        .context("scene output has no file name")?;
    let staged_manifest = staged.path().join(output_name);
    fs::write(&staged_manifest, &text)
        .with_context(|| format!("failed to stage plaque scene {}", staged_manifest.display()))?;
    ...
    // The scene manifest is the bundle's commit marker.
...
pub fn export_trajectory(args: ExportTrajectoryArgs) -> Result<()> {
    ...
    let output = args
        ...
        .clone()
        .map(Ok)
        .unwrap_or_else(|| workspace::trajectory_path(&source))?;
    let analyzed_plaque = pack
        .manifest
        .scenes
        .as_ref()
        .and_then(|inputs| inputs.surface_id.as_deref());
    let plaque = export_surface_id(args.surface.as_deref(), analyzed_plaque);
    let frames = pack
        .motion
        .iter()
        .map(|sample| {
        ...
        .collect::<Vec<_>>();
    let contents =
        trajectory_document(&plaque, &pack.manifest.source.sha256, &frames, args.locked)?;
    write_scene(&output, &contents, args.force)?;
    let authority = if args.locked { "locked" } else { "guided" };
    println!(
        "surface trajectory: {} ({} {authority} frames)",
        output.display(),
        frames.len()
    );
    Ok(())
...
fn export_surface_id(requested: Option<&str>, analyzed: Option<&str>) -> String {
    requested.or(analyzed).unwrap_or("main").to_string()
}

#[cfg(test)]
mod tests {
    use super::export_surface_id;

    #[test]
    fn export_uses_the_analyzed_plaque_unless_overridden() {
        assert_eq!(export_surface_id(None, Some("left")), "left");
        assert_eq!(export_surface_id(Some("right"), Some("left")), "right");
        assert_eq!(export_surface_id(None, None), "main");
    }
}
