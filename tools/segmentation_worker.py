...
def stabilize_alpha(
    ...
    gray = [
        cv2.imread(str(path), cv2.IMREAD_GRAYSCALE) for path in frames[start : end + 1]
    ]
    flows = [
        (
...
def model_masks(request, frames, requested_device, output):
    ...
    started = time.monotonic()
    if role == "foreground":
        # Temporal stabilization is noise suppression, not matte calibration: it
        # blends neighboring frames so thin structures stop flickering. It applies
        # to optical mattes and to opaque semantic confidence alike; the Rust
        # compositor still owns the scene's support-to-solid calibration policy.
...
def constrain_to_authored_motion_envelope(request, probabilities):
    ...
    def box_at(frame):
        ...
        right = next(
            index for index, prompt in enumerate(prompts) if prompt["frame"] >= frame
        )
        left = right - 1
        before = prompts[left]
        after = prompts[right]
