...
Failed in-progress trees are deleted automatically. Analysis retains only its compact summary/diagnostics under `/tmp/plaque-forge/failures/<asset>/`, bounded to three runs and seven days. Every later Plaque Forge staging operation reaps expired/excess failure evidence across all assets. Active work and destination locks hold operating-system file leases, so cleanup can distinguish a live transaction from abandoned work without relying on PID visibility or process namespaces. Transaction-private lease files never enter a published bundle. Destination-side incoming/backup names are deterministic, so a later run can restore or remove them after an interrupted publication. `scripts/cleanup_work.sh --yes` safely removes obsolete pre-transaction partial directories/publication siblings and completed-worker request files.
...
`./scripts/reset_analysis.sh --yes` is the only high-level command intended to wipe all generated scene-analysis caches. It resolves and verifies the deletion root as this repository's `assets/analysis/` and does not delete `assets/scenes`, source videos, plaque assets, `output/`, or `/tmp/plaque-forge-python`.
